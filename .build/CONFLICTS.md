...
## K. Found while building the release pipeline (2026-09-18)

| # | Spec said | Reality | How we handle it |
|---|---|---|---|
| C-020 | PACKAGING_SPEC section 6 releases npm in two moves: publish under the `next` tag, then a second approval moves `latest` onto it. | **npm's OIDC signs `npm publish` and `npm stage publish`, and nothing else.** `npm dist-tag add` is not one of the supported commands, so the second job could only have authenticated with a long-lived token stored in the repository -- reintroducing exactly the credential trusted publishing exists to remove, and holding it for the more dangerous of the two operations. | Staged publishing instead. `release.yml` runs `npm stage publish`: the version is uploaded but **not installable by anyone** until a human approves it on npmjs, which requires a 2FA challenge that no CI token can satisfy. This is a stronger gate than the spec's, not a weaker one -- under `@next` a mistake is public and merely unadvertised, while a staged mistake is invisible and `npm stage reject` spends no version number. The `npm-latest` job and environment are gone; `npm-next` becomes `npm-stage`. Needs npm 11.15.0+, which `release.yml` pins. **One exception, from the docs: you cannot stage a brand-new package.** Neither can you configure trusted publishing for one, so both exceptions end at the same moment and the workflow keys them off one condition -- `NPM_TOKEN` set means the first release and `npm publish`; absent means every release after it and `npm stage publish`. Approved by the owner, 2026-09-18, in chat. |

| C-021 | WORKFLOW Phase 3 requires the automation loop to be exercised by `scripts/loop-lab.mjs` in CI, and it was -- on Windows, where it passed. | **The release pipeline's first dry run failed on Linux.** `scripts/fake-claude.mjs` carries a shebang and is spawned directly, but git had it at mode `100644`, so Linux answered `EACCES` and no dispatch ever started. Windows ignores the executable bit entirely, which is why every local run passed and nothing in `pnpm check` noticed. | `git update-index --chmod=+x` on the stand-in. Found by running the release workflow in dry-run mode before spending a version number -- the same lesson as C-017 and C-018, on the one file whose whole job is to be run. |
| C-022 | PACKAGING_SPEC section 6: "publish PyPI bang **trusted publishing (OIDC)**; TestPyPI truoc." | The owner does not use TestPyPI and publishes straight to PyPI. | The `testpypi` job and environment are gone. That removed the only place a broken wheel could be found without spending a version number, so what it guarded is checked before the gate instead: `pypa/gh-action-pypi-publish` runs `twine check` on every distribution, and the wheels job installs the wheel it just built and runs `delphi --version`. The smoke test covers one wheel of six -- only the runner's own platform can be executed -- but the other five share every build step with it, so it proves the recipe produces something that runs. That is the failure C-021 already was. Deleting the job also deleted the dry-run guard the `pypi` job was inheriting through `needs: testpypi`, so `pypi` carries its own `if` now. Decided by the owner, 2026-09-18, in chat. |

## L. Found by the first real release (2026-09-18)

| # | Spec said | Reality | How we handle it |
|---|---|---|---|
| C-023 | PACKAGING_SPEC section 6 and the runbook both treat "a granular token with write access" as enough for the first npm publish, and C-020 requires 2FA on the account so a staged release can be approved later. | **Those two requirements collide, and only on the first release.** 2FA on the account means npm answers a token publish with `EOTP` -- it wants a one-time password, and CI has nobody to ask. The run got as far as signing provenance and writing it to the transparency log before failing, so the failure looks late and total; nothing reached the registry and the version number was not spent. PyPI, in the same run, published fine -- OIDC has no such problem. | The token needs the **Bypass 2FA** capability, which takes precedence over the account setting for publishing. The runbook now says so where the token is created, with the reason, rather than listing it as one field among five. Worth knowing: npm retires bypass-2FA tokens for direct publishing in January 2027, which is survivable only because this token exists for exactly one release and is deleted afterwards. The real lesson is narrower than it looks -- every later release is staged under OIDC, where neither 2FA nor a token is involved at all. |
| C-024 | WORKFLOW Phase 3 treats a green dry run as evidence the release pipeline works, and three of them were green. | **The one job a dry run always skips is the one that was broken.** `github-release` failed twice over: all six binaries were named `delphi`, so GitHub -- which names an asset after its file -- took the first and rejected the rest, and `needs.verify.outputs.version` was empty because the `needs` context exposes only *direct* dependencies, so the release was titled "v". Both would have been visible in ten seconds of running the job and were invisible in three dry runs and every reading of the file. | Binaries carry their platform from the moment they are compiled -- which also makes `SHA256SUMS` list the names people download, without which `sha256sum -c` helped nobody -- and `verify` is a direct need. The v0.1.0 release was assembled by hand from the run's artifacts, because a re-run uses the workflow file the run started with. The wider lesson: `if:` conditions that skip a job in a dry run also hide it, so a job that only runs for real is a job that has never been tested. |
| C-025 | WORKFLOW Phase 6 wires `changesets/action` with `version-script: pnpm changeset version && pnpm sync-version`, and PACKAGING_SPEC section 1 says `sync-version` keeps "npm, PyPI, the plugin and the GitHub release" in lockstep. | **Neither was true, and neither could be seen until a changeset existed.** The action splits `version-script` into argv and runs it with no shell, so the `&&` arrived as an argument and changesets exited with "Unused args: `&&`". And `sync-version` never touched the plugin manifests at all despite its own header saying it did -- so the version pull request would have carried a bumped `package.json` beside a stale `plugin.json`, and CI's "generated files are current" job would have failed on the very pull request whose job is to bump the version. The workflow had run green on every push to main for days, doing nothing, because there was never a changeset to act on. | The two steps are one `pnpm version-packages` script, and `sync-version` now patches the three manifests that carry a version. Patching the line rather than regenerating means bumping a version still needs no build. The wider lesson matches C-024: a workflow that runs green while having nothing to do has not been tested, and `if:`-style emptiness hides as well as a skip does. |
